...
public enum Balancing {
  ...
  private static void checkIfBalanced(final String text, final Stack<String> stack) throws UnbalancedException {
    for (final Token token : new Tokenizer(new StringReader(text)))
      if (!closers.contains(token.text)) {
        if (map.keySet().contains(token.text))
          stack.push(token.text);
      } else {
        if (stack.isEmpty() || !map.get(stack.peek()).equals(token.text))
