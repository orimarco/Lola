...
public class $Find extends Keyword implements RegExpable {
  public $Find(final Token token) {
    super(token);
    expectedElaborators = new ArrayList<>(Arrays.asList(new String[] { "$replace", "$delete", "$append", "$prepend", "$description", "$note", "$log",
    ...
    switch (state) {
      case Snippet:
        if (b instanceof SnippetToken || b instanceof TriviaToken)
          return true;
        return !(b instanceof $Find) && !(b instanceof UnacceptableGeneratingKeyword) && (!(b instanceof Elaborator) || expectedElaborators.contains(b.name()));
      case List:
        return !(b instanceof $Find) && !(b instanceof UnacceptableGeneratingKeyword) && (!(b instanceof Elaborator) || expectedElaborators.contains(b.name()));
      case Elaborators:
