...
public class $ForEach extends Keyword implements GeneratingKeyword {
	...
	public String generate(final PythonAdapter a) {
		final int iterations = a.forEachBeforeAndGetIterationsNum(
				snippet.getText().substring(snippet.getText().indexOf('(') + 1, snippet.getText().lastIndexOf(')')));
		if (iterations == 0) {
			a.forEachAfter();
		...
		for (int i = 0; i < iterations; ++i) {
			...
			if (i > 0)
				$ += separator;
			$ += list.stream()
					.map(x -> !(x instanceof GeneratingKeyword) ? x.token.text : ((GeneratingKeyword) x).generate(a))
					.reduce((x, y) -> x + y).get();
		}
		a.forEachAfter();
		return $;
