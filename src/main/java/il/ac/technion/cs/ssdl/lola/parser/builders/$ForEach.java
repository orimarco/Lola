...
public class $ForEach extends Keyword implements GeneratingKeyword {
	public $ForEach(final Token token) {
		...
		expectedElaborators = new ArrayList<>(
				Arrays.asList(new String[]{"$ifNone", "$separator"}));
	}
	...
	public boolean accepts(final Node b) {
		...
		switch (state) {
			...
			case List :
				return !(b instanceof Keyword) || expectedElaborators.contains(b.name())
						|| b instanceof GeneratingKeyword;
			case Snippet :
				return true;
			default :
				return false;
	...
	public Node done() {
		if ("\n".equals(list.get(list.size() - 1).text())
				|| "\r".equals(list.get(list.size() - 1).text())
				|| "\r\n".equals(list.get(list.size() - 1).text()))
	...
	public String generate(final PythonAdapter a) {
		final int iterations = a.forEachBeforeAndGetIterationsNum(
				snippet.getText().substring(snippet.getText().indexOf('(') + 1,
						snippet.getText().lastIndexOf(')')));
		// System.out.println(iterations);
		if (iterations == 0) {
			a.forEachAfter();
			return (($ifNone) elaborators.stream().filter(x -> x instanceof $ifNone)
					.findFirst().get()).generate(a);
		}
		final Optional<Elaborator> sep = elaborators.stream()
				.filter(x -> x instanceof $separator).findFirst();
		final String separator = !sep.isPresent()
				? ""
				: (($separator) sep.get()).separator;
		String $ = "";
		for (int i = 0; i < iterations; ++i) {
			...
			if (i > 0)
				$ += separator;
			$ += list.stream()
					.map(x -> !(x instanceof GeneratingKeyword)
							? x.token.text
							: ((GeneratingKeyword) x).generate(a))
					.reduce((x, y) -> x + y).get();
		}
		a.forEachAfter();
		return $;
