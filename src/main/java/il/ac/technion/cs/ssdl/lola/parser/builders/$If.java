...
public class $If extends Keyword implements GeneratingKeyword {
  public $If(final Token token) {
    super(token);
    expectedElaborators = new ArrayList<>(Arrays.asList(new String[] { "$else", "$elseIf" }));
  }
  ...
  @Override public String generate(final PythonAdapter a) {
    ...
    if (a.evaluateBooleanExpression(snippet.getText()))
      ...
      for (final Elaborator e : elaborators)
        if (e instanceof $else)
          toExecute = e.list;
        else if (a.evaluateBooleanExpression(e.snippet.getText())) {
          toExecute = e.list;
          break;
        }
    return toExecute.stream().map(n -> n.token.text).reduce("", (s1, s2) -> s1 + s2);
  }
