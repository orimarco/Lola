...
public abstract class ExecutableElaborator extends Elaborator {
  ...
  @Override public boolean accepts(final AST.Node b) {
    return state == Automaton.List && !(b instanceof Elaborator) && !(b instanceof $Find);
  }
  ...
  @Override public Node done() {
    if (!list.isEmpty() && "enter".equals(list.get(list.size() - 1).categoryName()))
      list.remove(list.size() - 1);
    return this;
  }
  abstract public void execute(Chain<Bunny, Lexi>.Interval i, PythonAdapter a, Parser p);
}
